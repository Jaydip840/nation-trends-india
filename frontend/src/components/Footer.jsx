    ...
    <footer className="bg-slate-900 text-white mt-auto border-t-[10px] border-primary-red selection:bg-white selection:text-slate-900">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-12">
        <div className="grid grid-cols-2 lg:grid-cols-12 gap-x-8 gap-y-12">
          ...
          {/* BRAND COLUMN */}
          <div className="lg:col-span-4 space-y-6 col-span-2">
          ...
          <div className="lg:col-span-2 space-y-4 col-span-1">
            <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-primary-red italic mb-3">Quick Links</h4>
            <ul className="space-y-2">
              {['Home', 'About Us', 'Contact'].map((item) => (
                <li key={item}>
                    <Link 
                        to={item === 'Home' ? '/' : (item === 'About Us' ? '/about' : `/${item.toLowerCase().replace(' ', '-')}`)} 
                        className="text-[10px] md:text-[11px] font-black uppercase tracking-widest text-slate-300 hover:text-white hover:pl-2 transition-all duration-300"
                    >
                        {item}
                    </Link>
                </li>
          ...
          <div className="lg:col-span-3 space-y-4 col-span-1">
            <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-primary-red italic mb-3">Categories</h4>
            <div className="grid grid-cols-1 gap-y-2">
              {['India', 'World', 'Politics', 'Business', 'Technology', 'Entertainment', 'Sports'].map((cat) => (
                ...
                >
                    <span className="text-[10px] md:text-[11px] font-black uppercase tracking-widest text-slate-300 group-hover:text-white transition-colors">
                        {cat}
                    </span>
                    <FiArrowUpRight className="text-slate-600 group-hover:text-primary-red transition-all transform group-hover:translate-x-1 group-hover:-translate-y-1" size={10} />
                </Link>
              ))}
            </div>
          </div>
          ...
          {/* CONTACT INFO */}
          <div className="lg:col-span-3 space-y-4 col-span-2 md:col-span-1">
        ...
        {/* BOTTOM BAR */}
        <div className="mt-12 pt-8 border-t border-slate-800 flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-x-8 gap-y-4">
            <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em] w-full md:w-auto text-center md:text-left mb-2 md:mb-0">
